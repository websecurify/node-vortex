...
Provider.prototype.import = function (vmUrl, vmId, callback) {
	...
	if (self.importStates.hasOwnProperty(vmUrl)) {
		if (self.importStates[vmUrl] == 'importing') {
			var milliseconds = 10000;
			var args = arguments;
			
			setTimeout(function () {
				self.import.apply(self, args);
			}, milliseconds);
		} else
		if (self.importStates[vmUrl] == 'imported') {
			callback();
		} else
		if (self.importStates[vmUrl]) {
			callback(self.importStates[vmUrl]);
		} else {
			callback(helpers.e('undefined state for import from', helpers.q(vmUrl)));
		}
		
		return;
	}
	
	self.importStates[vmUrl] == 'importing';
	...
	callback = (function (callback) {
		return function (err) {
			if (err) {
				self.importStates[vmUrl] = err;
				
				return callback(err);
			}
			
			self.importStates[vmUrl] = 'imported';
