...
Provider.prototype.import = function (vmUrl, vmId, callback) {
	...
	Task.prototype.run = function (callback) {
		...
		logger.debug('import', helpers.q(self.vmUrl), 'into', helpers.q(self.vmId));
		
		var spec;
		
		try {
			spec = url.parse(self.vmUrl);
		} catch (e) {
			return callback(helpers.e('cannot parse url', helpers.q(self.vmUrl)));
		}
		
		if (['file:', 'http:', 'https:'].indexOf(spec.protocol) < 0) {
			return callback(helpers.e('unsupported scheme for url', helpers.q(self.vmUrl)));
		}
		...
		if (spec.protocol == 'file:') {
			...
			self.command('import', localPath, '--vsys', '0', '--vmname', self.vmId, function (err, output, code) {
				if (err) {
					return callback(err);
				}
				
				if (code > 0) {
					return callback(helpers.e('cannot import', helpers.q(localPath), 'with name', helpers.q(self.vmId)));
				}
			...
			download.get(self.vmUrl, localPath, function (err) {
				if (err) {
					...
					return callback(helpers.e('cannot download', helpers.q(self.vmUrl), 'for', helpers.q(self.vmId)));
				}
				...
				self.command('import', localPath, '--vsys', '0', '--vmname', self.vmId, function (err, output, code) {
					...
					if (code > 0) {
						return callback(helpers.e('cannot import', helpers.q(localPath), 'into', helpers.q(self.vmId)));
					}
