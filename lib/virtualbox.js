...
Provider.prototype.import = function (url, handle, callback) {
	var Task = function (url, handle) {
		this.url = url;
		this.handle = handle;
	};
	...
	Task.prototype.run = function (callback) {
		...
		logger.debug('import', helpers.q(self.url), 'into', helpers.q(self.handle));
		
		var spec;
		
		try {
			spec = url.parse(self.url);
		} catch (e) {
			return callback(helpers.e('cannot parse url', helpers.q(self.url)));
		}
		
		if (['file:', 'http:', 'https:'].indexOf(spec.protocol) < 0) {
			return callback(helpers.e('unsupported scheme for url', helpers.q(self.url)));
		}
		...
		if (spec.protocol == 'file:') {
			...
			self.exec('import', localPath, '--vsys', '0', '--vmname', self.handle, function (err, output, code) {
				if (err) {
					return callback(err);
				}
				
				if (code > 0) {
					return callback(helpers.e('cannot import', helpers.q(localPath), 'with name', helpers.q(self.handle)));
				}
			...
			download.get(self.url, localPath, function (err) {
				if (err) {
					...
					return callback(helpers.e('cannot download', helpers.q(self.url), 'for', helpers.q(self.handle)));
				}
				...
				self.exec('import', localPath, '--vsys', '0', '--vmname', self.handle, function (err, output, code) {
					...
					if (code > 0) {
						return callback(helpers.e('cannot import', helpers.q(localPath), 'into', helpers.q(self.handle)));
					}
...
Provider.prototype.status = function (nodeName, callback) {
	...
	var obtainInfo = function (node, callback) {
		...
		self.exec('showvminfo', handle, function (err, output, code) {
			if (err) {
				return callback(err);
			}
	...
	var completeInfo = function (info, callback) {
		...
		self.exec('guestproperty', 'enumerate', info.name, function (err, output, code) {
			if (err) {
				return callback(err);
			}
	...
	async.waterfall(
		...
		function (err, info) {
			...
			callback(null, info.state, info.id, info.address);
		}
	);
};
...
Provider.prototype.boot = function (nodeName, callback) {
	...
	var obtainStatus = function (node, callback) {
		self.status(node, function (err, state, id, address) {
			if (err) {
				return callback(err);
			}
	...
	var configureNetworking = function (node, callback) {
		...
		self.exec('modifyvm', [handle].concat(interfaces), function (err, output, code) {
			if (err) {
				return callback(err);
			}
