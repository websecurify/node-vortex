...
Provider.prototype.import = function (url, handle, callback) {
	...
	Task.prototype.run = function (callback) {
		...
		if (spec.protocol == 'file:') {
			...
			self.command('import', localPath, '--vsys', '0', '--vmname', self.handle, function (err, output, code) {
				if (err) {
					return callback(err);
				}
...
Provider.prototype.bootstrap = function (nodeName, callback) {
	...
	if (node.hasOwnProperty('expose')) {
		...
		Object.keys(node.expose).forEach(function (source) {
			...
			node.roost.bootstrap.push('sudo mount.vboxsf ' + roost.shell.quote(share) + ' ' + roost.shell.quote(destination));
		});
	}
