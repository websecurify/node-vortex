...
Provider.prototype.rewire = function (callback) {
	...
	var ensureVboxnet0 = function (callback) {
		self.command('list', 'hostonlyifs', function (err, output, code) {
			if (err) {
				return callback(err);
			}
			
			if (code > 0) {
				return callback(helpers.e('cannot list hostonly network interfaces'));
			}
			
			if (output.length == 0) {
				self.command('hostonlyif', 'create', function (err, output, code) {
					if (err) {
						return callback(err);
					}
					
					if (code > 0) {
						return callback(helpers.e("cannot create hostonly interface for", helpers.q('vboxnet0')));
					}
	...
	var setupVboxnet0Network = function (callback) {
		self.command('hostonlyif', 'ipconfig', 'vboxnet0', '--ip', '10.100.100.1', '--netmask', '255.255.255.0', function (err, output, code) {
			...
			if (code > 0) {
				return callback(helpers.e('cannot configure network interface for', helpers.q('vboxnet0')));
			}
	...
	var removeVboxnet0Dhcp = function (callback) {
		self.command('dhcpserver', 'remove', '--netname', 'vboxnet0', function (err, output, code) {
			if (err) {
				return callback(err);
			}
			
			if (code > 0) {
				logger.debug('cannot remove dhcp server for', helpers.q('vboxnet0'));
			}
	...
	var setupVboxnet0Dhcp = function (callback) {
		self.command('dhcpserver', 'add', '--ifname', 'vboxnet0', '--ip', '10.100.100.100', '--netmask', '255.255.255.0', '--lowerip', '10.100.100.101', '--upperip', '10.100.100.254', '--enable', function (err, output, code) {
			if (err) {
				return callback(err);
			}
			
			if (code > 0) {
				return callback(helpers.e('cannot configure dhcp server for', helpers.q('vboxnet0')));
			}
	...
	var removeVortexDhcp = function (callback) {
		self.command('dhcpserver', 'remove', '--netname', 'vortex', function (err, output, code) {
			if (err) {
				return callback(err);
			}
			
			if (code > 0) {
				logger.debug('cannot remove dhcp server for', helpers.q('vortex'));
			}
	...
	var setupVortexDhcp = function (callback) {
		self.command('dhcpserver', 'add', '--netname', 'vortex', '--ip', '10.200.200.100', '--netmask', '255.255.255.0', '--lowerip', '10.200.200.101', '--upperip', '10.200.200.254', '--enable', function (err, output, code) {
			...
			if (code > 0) {
				return callback(helpers.e('cannot configure dhcpp server for', helpers.q('vortex')));
			}
