	...
	client.describeInstances(options, function (err, result) {
		...
		instances
			...
			.forEach(function (instance) {
				logger.warn('duplicate node', helpers.q(nodeName), 'with instance id', helpers.q(instance.id), 'detected');
			});
		...
		logger.debug('node', helpers.q(nodeName), 'with instance id', helpers.q(selectedInstance.id), 'has state', helpers.q(state));
...
Provider.prototype.boot = function (nodeName, callback) {
	...
	var runInstance = function (node, callback) {
		...
		client.runInstances(options, function (err, result) {
			...
			logger.debug('ran instances', instances);
			...
			instances
				...
				.forEach(function (instance) {
					logger.warn('duplicate node', helpers.q(node), 'with instance id', helpers.q(instance.id), 'detected');
				});
	...
	var mapTags = function (node, instanceId, callback) {
		...
		logger.debug('create tags with options', options);
...
Provider.prototype.halt = function (nodeName, callback) {
	...
	var unmapTags = function (node, instanceId, callback) {
		...
		logger.debug('delete tags with options', options);
