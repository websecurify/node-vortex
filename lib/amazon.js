...
Provider.prototype.createError = function (error, name) {
	if (error.code == 'NetworkingError') {
		...
		message = message.replace(/required\skey\s'(\w+)'/, function (match, group) {
			return "required key '" + helpers.camel(group) + "'";
		});
...
Provider.prototype.status = function (name, node, callback) {
	logger.debug('get status for node', name, node);
	...
	client.describeInstances(options, function (err, result) {
		...
		logger.debug('node', name, 'with instance id', selectedInstance.id, 'has state', state);
...
Provider.prototype.boot = function (name, node, callback) {
	logger.debug('boot node', name, node);
		...
		client.runInstances(options, function (err, result) {
			...
			if (instances.length >= 2) {
				instances
					...
					.forEach(function (instance) {
						logger.warn('multiple instances with the same configuration detected:', name, instance.id);
					});
			}
...
Provider.prototype.halt = function (name, node, callback) {
	logger.debug('halt node', name, node);
...
Provider.prototype.shellSpec = function (name, node, callback) {
	logger.debug('shell spec node', name, node);
	...
	this.status(name, node, function (err, state, instanceId, dnsName) {
		...
		if (state == 'booting') {
			...
			logger.debug('repeat status for node', name, 'in', milliseconds, 'milliseconds');
		...
		logger.debug('check for ssh port open for node', name);
		...
		portchecker.isOpen(22, dnsName, function (isOpen) {
			if (isOpen) {
				...
				logger.debug('final spec for node', name, 'is', spec);
				...
				logger.debug('repeat check for ssh port open for node', name, 'in', milliseconds, 'milliseconds');
