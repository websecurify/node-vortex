...
function Provider(manifest) {
	if (manifest.hasOwnProperty('namespace')) {
...
Provider.prototype.shellSpec = function (name, node, callback) {
	...
	var options = self.extractShellOptions(node);
	...
	self.status(name, node, function (err, state, instanceId, address) {
		...
		portchecker.isOpen(22, address, function (isOpen) {
			if (isOpen) {
				...
				setTimeout(function () {
					portchecker.isOpen(22, address, callee);
				}, milliseconds);
			}
		})
	});
