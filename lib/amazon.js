...
function Provider(manifest) {
	...
	this.shellOptions = ['username', 'password', 'privateKey', 'passphrase'];
...
Provider.prototype.extractOptions = function (list, source) {
	...
	if (source.hasOwnProperty('amazon')) {
		...
		list.forEach(function (option) {
			if (amazon.hasOwnProperty(option)) {
				options[option] = amazon[option];
			}
		});
	}
...
Provider.prototype.status = function (name, node, callback) {
	...
	client.describeInstances(options, function (err, result) {
		...
		return callback(null, state, instances[0].id, instances[0].dnsName);
	});
};
