...
function shell(opt, manifest, provider, name, node, callback) {
	...
	provider.shellSpec(name, node, function (err, spec) {
		...
		connection.connect(spec, function (err) {
			...
			logger.info('shell connected to node', helpers.q(name));
			...
			connection.shell(function (err) {
				...
				logger.verbose('shell returned from node', helpers.q(name));
				...
				connection.disconnect({}, function (err) {
					...
					logger.info('shell disconnected from node', helpers.q(name));
...
function provision(opt, manifest, provider, name, node, callback) {
	logger.verbose('invoke provider.provision for node', helpers.q(name));
	...
	provider.provision(name, node, function (err) {
		...
		provider.shellSpec(name, node, function (err, spec) {
			...
			if (manifest.hasOwnProperty('nodes')) {
				...
				if (roostManifest.hasOwnProperty('commands')) {
					if (Array.isArray(roostManifest.commands)) {
						mapCommand = function (command) {
							roostManifest.commands.slice(0, 0, command);
						};
					} else {
						mapCommand = function (command) {
							roostManifest[command] = true;
						};
					}
				} else {
					roostManifest.commands = [];
					
					mapCommand = function (command) {
						roostManifest.commands.push(command);
					};
				}
				
				Object.keys(manifest.nodes).forEach(function (key) {
					if (key == name) {
						return;
					}
					
					var node = manifest.nodes[key];
					
					if (!node.hasOwnProperty('meta') || !node.meta.hasOwnProperty('address')) {
						logger.error('node', helpers.q(key), 'does not expose address');
						
						return;
					}
					
					var nodeName = key;
					var nodeAddress = node.meta.address;
					var q = roost.shell.quote;
					
					mapCommand('sudo mkdir -p /etc/vortex/hosts/;sudo echo ' + q(nodeAddress) + ' > ' + q('/etc/vortex/hosts/' + nodeName));
				});
			} else {
				logger.error('manifest does not contain any nodes when provisioning node', helpers.q(name));
			}
			...
			logger.debug('starting roost with ops', roostOpt);
